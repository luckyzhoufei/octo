...
def log(t, eps=1e-20):
    # Ensure that the lowest value input in log function should not smaller than 1e-20
...
class ContinuousTimeGaussianDiffusion(nn.Module):
    ...
    def q_sample(self, x_start, times, noise=None):   # 加噪
    ...
    def random_times(self, batch_size):
        # times are now uniform from 0 to 1 with shape of (batch_size,)
        return torch.zeros((batch_size,), device=self.device).float().uniform_(0, 1)

    def p_losses(self, x_start, times, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))

        x, log_snr = self.q_sample(x_start=x_start, times=times, noise=noise)  # 加噪
